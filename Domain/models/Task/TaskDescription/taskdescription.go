package task

import (
	"fmt"
	"unicode/utf8"
)

const (
	MIN_LENGTH = 1
	MAX_LENGTH = 200
)

type TaskDescriptionInterface interface {
	Value() string
}

type TaskDescription struct {
	value string
}

func NewTaskDescription(value string) (TaskDescriptionInterface, error) {
	if err := validate(value); err != nil {
		return nil, err
	}
	return &TaskDescription{value}, nil
}

func (t *TaskDescription) Value() string {
	return t.value
}

func validate(value string) error {
	if utf8.RuneCountInString(value) < MIN_LENGTH {
		return fmt.Errorf("TaskDescription must be at least %d characters long", MIN_LENGTH)
	}
	if utf8.RuneCountInString(value) > MAX_LENGTH {
		return fmt.Errorf("TaskDescription must be no more than %d characters long", MAX_LENGTH)
	}
	return nil
}
